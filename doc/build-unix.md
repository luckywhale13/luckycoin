...
You must install required dependencies to build a basic Luckycoin daemon, optional dependencies may vary according to your requirements.
...
Luckycoin Core's released binaries are built using the "depends system", which
...
According to installed dependencies, the following steps will compile `luckycoind`, `luckycoin-cli` and `luckycoin-qt`.
...
By default, **Luckycoin Core expects BerkeleyDB 5.3**.
...
Otherwise, you can build Luckycoin Core from self-compiled [depends](/depends/README.md).
...
least 1.5 GB of memory available when compiling Luckycoin Core.
