...
Luckycoin Core 1.8 introduces AuxPoW from block 371,337. AuxPoW is a technology
which enables miners to submit work done while mining other coins, as work
on the Luckycoin block chain. Luckycoin Core 1.8 also enables payment protocol
support for Luckycoin (note that Luckycoin's implementation of payment protocol is
...
as soon as possible afterwards in order to be able to continue to use Luckycoin.
Users remaining on the previous client **will not** be able to receive or spend
Luckycoin (with the exception of with other nodes who have not upgraded).
...
AuxPoW benefits Luckycoin in two ways; firstly, the effective hashrate of the coin is
...
multipools from switching between Luckycoin and other Scrypt coins.
...
In comparison to the AuxPoW implementation in United Scrypt Coin, Luckycoin uses a
different coinbase transaction input script format which includes block height in
compliance with BIP0034.

## Payment Protocol

Luckycoin Core 1.8 adds payment protocol support, which is used to streamline payments
...
The key differencs between the Bitcoin and Luckycoin payment protocols are that
the Luckycoin payment request uses a "genesis" field containing the hash of the network's
...
of Luckycoin compared to Bitcoin. This should lead to less false positives.
