Luckycoin Core version 1.14.5 is now available from:

  <https://github.com/luckywhale13/luckycoin/releases/tag/v1.14.5/>

This is a new minor version release, including important security updates and
changes to network policies. All Luckycoin Core users, miners, services, relay
...
Luckycoin Core is extensively tested on Ubuntu Server LTS, Intel-based macOS
and Windows 10.
...
most Luckycoin Core users.

### Remote Code Execution in Luckycoin QT (CVE-2021-3401)
...
**Luckycoin QT users are urged to please update their installations to this
...
leakage of sensitive information when Luckycoin Core crashes on Unix platforms.
The vulnerability is patched for systems that run a Linux kernel equal to or
higher than 3.4.

**Luckycoin Core wallet users on Linux platforms are urged to please update to
this version.**
...
the Luckycoin network, following the reduction of relay and mining defaults in
...
* The user interface for selecting fees when transacting DOGE has been updated
  ...
  Luckycoin blocks are not full, typically all transactions are mined in the next
  block, and therefore the target estimation does not makes sense for Luckycoin.
...
The Berkley DB version used by Luckycoin Core has been updated to 5.3 (from 5.1)
...
performance of cryptographic routines within Luckycoin Core.
