...
Below are some notes on how to build Luckycoin Core for Windows.
...
Note that for WSL v1 the Luckycoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/luckycoin, AND not under, for example, /mnt/d/luckycoin. 
...
way. This will install to `c:\workspace\luckycoin`, for example:

    make install DESTDIR=/mnt/c/workspace/luckycoin
