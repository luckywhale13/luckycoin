...
The data directory is the default location where the Luckycoin Core files are stored.
...
Windows  | `%APPDATA%\Luckycoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for the `luckycoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `luckycoin.conf`       | User-defined configuration settings for `luckycoind` or `luckycoin-qt`; can be specified by `-conf` option
`./`               | `luckycoind.pid`        | Stores the process ID (PID) of `luckycoind` or `luckycoin-qt` while running; can be specified by `-pid` option
...
`luckycoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Luckycoin Core:
